...
func main() {
	...
	areaHeightNum := 2  // エリアの縦の数
	areaWidthNum := 2  // エリアのの横の数
	areaLength := 100  // エリアの一辺の長さ
	fileName := "exp_4.yaml"

	option := Option{
		FileName: fileName,
		AreaLength: areaLength,
		AreaHeightNum: areaHeightNum,
		AreaWidthNum: areaWidthNum,
		AreaCoords: []Coord{
...
func AreaDivider2(option Option) []Area {
	// workerNum分areaLengthを広げる
	
	areaHeightNum := option.AreaHeightNum
	areaWidthNum := option.AreaWidthNum
	areaLength := option.AreaLength   // m
	duplicateRate := option.DuplicateRate
	
	areas := []Area{}

	minLat := 0.0
	maxLat := minLat + float64(areaHeightNum) * float64(areaLength)
	minLon := 0.0
	maxLon := minLon + float64(areaWidthNum) * float64(areaLength)
	//maxLat, maxLon, minLat, minLon := GetCoordRange(areaCoords)
	for i := 0; i < areaHeightNum; i++ { // 横方向
		leftlon := (maxLon-minLon)*float64(i)/float64(areaHeightNum) + minLon
		rightlon := (maxLon-minLon)*(float64(i)+1)/float64(areaHeightNum) + minLon

		for k := 0; k < areaWidthNum; k++ { // 縦方向
			bottomlat := (maxLat-minLat)*float64(k)/float64(areaWidthNum) + minLat
			toplat := (maxLat-minLat)*(float64(k)+1)/float64(areaWidthNum) + minLat
