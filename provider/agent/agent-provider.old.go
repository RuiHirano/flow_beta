...
func NewWorkerAPI(simapi *util.SimAPI, servAddr string, nodeAddr string, cb *util.Callback ) *WorkerAPI {
	api := &WorkerAPI{
		SimAPI: simapi,
		ServerAddr: servAddr,
		NodeAddr: nodeAddr,
		sclientOptsWorker: map[uint32]*util.SclientOpt{
			uint32(api.ChannelType_CLOCK): &util.SclientOpt{
				ChType:       uint32(api.ChannelType_CLOCK),
				MBusCallback: util.GetClockCallback(simapi, cb),
				ArgJson:      fmt.Sprintf("{Client:WorkerProvider_Clock}"),
			},
			uint32(api.ChannelType_PROVIDER): &util.SclientOpt{
				ChType:       uint32(api.ChannelType_PROVIDER),
				MBusCallback: util.GetProviderCallback(simapi, cb),
				ArgJson:      fmt.Sprintf("{Client:WorkerProvider_Provider}"),
			},
			uint32(api.ChannelType_AGENT): &util.SclientOpt{
				ChType:       uint32(api.ChannelType_AGENT),
				MBusCallback: util.GetAgentCallback(simapi, cb),
				ArgJson:      fmt.Sprintf("{Client:WorkerProvider_Agent}"),
			},
			uint32(api.ChannelType_AREA): &util.SclientOpt{
				ChType:       uint32(api.ChannelType_AREA),
				MBusCallback: util.GetAreaCallback(simapi, cb),
...
func (wa *WorkerAPI) Connect(pname string) error {
	channelTypes = []uint32{}
	ni = sxutil.NewNodeServInfo()
	util.RegisterNodeLoop(ni, wa.NodeAddr, pname, channelTypes)

	// Register Synerex Server
	client = util.RegisterSynerexLoop(wa.ServerAddr)

	// Register Callback
	util.RegisterSXServiceClients(ni, client, wa.SclientOpts)
	logger.Success("Finish Connect")
}

func (wa *WorkerAPI) RegisterSimulator() error {
	sclient := wa.SclientOpts[uint32(api.ChannelType_PROVIDER)].Sclient
...
// filters -> targets
func (wa *WorkerAPI) ForwardClock() error {
	...
	wa.SimAPI.ForwardClockInitRequest(sclient, targets)      // init
}
...
func main() {
	...
	simapi = api.NewSimAPI(myProvider)
	workerAPI = NewWorkerAPI(simapi, *servaddr, *nodeAddr)
	workerAPI.Connect("AgentProvider")
	workerAPI.RegisterSimulator()
